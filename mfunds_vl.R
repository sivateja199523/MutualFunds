...
rawData_vl <- fread("Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/mf_vl_mice.csv"
                    , sep=",", header=T, 
                    strip.white = T, na.strings = c("NA","NaN","","?"))
#View(rawData_bl)
mf_bl<-rawData_vl
final_data_vl <-rawData_vl
#View(mf_bl)
...
final_data_vl$hclust <- groups_bl  #Adding hclusters to data.
#View(final_data_bl)
...
summary(pca_bl)
#DB scan
kNNdistplot(df_bl_mf, k =15) #k and minPts will be same, Considering PCA=14 which given 90% Cumulative Proportion
abline(h=7, col="red")
...
final_data_vl$dbscan<- db_bl$cluster
dbscan_results_vl<-aggregate(final_data_vl[, 2:37], list(final_data_vl$dbscan), mean)
#K means clustering
...
final_data_vl$kmeans <- k.means.fit_bl$cluster
...
write.table(final_data_vl, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/final_data_vl.csv", sep=",", row.names=F)
write.table(dbscan_results_vl, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/dbscan_results_vl.csv", sep=",", row.names=F)
library(fpc)
