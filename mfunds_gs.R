...
rawData_gs <- fread("Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/mf_gs_mice.csv"
                    , sep=",", header=T, 
                    strip.white = T, na.strings = c("NA","NaN","","?"))
#View(rawData_bl)
mf_bl<-rawData_gs
final_data_gs <-rawData_gs
#View(mf_bl)
...
final_data_gs$hclust <- groups_bl  #Adding hclusters to data.
View(final_data_bl)
...
abline(h=7, col="red")
db_bl <- dbscan(df_bl_mf, eps=4, minPts=15)  #Need to perform pca to get the minpts, eps is from the above plot
db_bl
clusplot(df_bl_mf, db_bl$cluster, main='2D representation of the Cluster solution',
         color=TRUE, shade=TRUE,
         labels=2, lines=0)
hullplot(df_bl_mf, db_bl)
final_data_gs$dbscan<- db_bl$cluster
dbscan_results_gs<-aggregate(final_data_gs[, 2:37], list(final_data_gs$dbscan), mean)
#K means clustering
...
final_data_gs$kmeans <- k.means.fit_bl$cluster
...
write.table(final_data_gs, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/final_data_gs.csv", sep=",", row.names=F)
write.table(dbscan_results_gs, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/dbscan_results_gs.csv", sep=",", row.names=F)
library(fpc)
