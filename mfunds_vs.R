...
rawData_vs<- fread("Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/mf_vs_mice.csv"
                    , sep=",", header=T, 
                    strip.white = T, na.strings = c("NA","NaN","","?"))
rawData_vs<-select(rawData_vs,-price_cashflow,-fund_treynor_ratio_3years,-fund_treynor_ratio_5years,-fund_treynor_ratio_10years)
#View(rawData_bl)
mf_bl<-rawData_vs
final_data_vs <-rawData_vs
#View(mf_bl)
...
final_data_vs$hclust <- groups_bl  #Adding hclusters to data.
#View(final_data_bl)
...
abline(h=7, col="red")
db_bl <- dbscan(df_bl_mf, eps=3, minPts=15)  #Need to perform pca to get the minpts, eps is from the above plot
db_bl
clusplot(df_bl_mf, db_bl$cluster, main='2D representation of the Cluster solution',
         color=TRUE, shade=TRUE,
         labels=2, lines=0)
hullplot(df_bl_mf, db_bl)
final_data_vs$dbscan<- db_bl$cluster
dbscan_results_vs<-aggregate(final_data_vs[, 2:33], list(final_data_vs$dbscan), mean)
#K means clustering
...
final_data_vs$kmeans <- k.means.fit_bl$cluster
...
write.table(final_data_vs, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/final_data_vs.csv", sep=",", row.names=F)
write.table(dbscan_results_vs, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/dbscan_results_vs.csv", sep=",", row.names=F)
library(fpc)
