...
rawData_vm <- fread("Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/mf_vm_mice.csv"
                    , sep=",", header=T, 
                    strip.white = T, na.strings = c("NA","NaN","","?"))
rawData_vm<-select(rawData_vm,-price_earnings,-fund_treynor_ratio_3years,-fund_treynor_ratio_5years,-fund_treynor_ratio_10years)
#View(rawData_bl)
mf_bl<-rawData_vm
final_data_vm <-rawData_vm
#View(mf_bl)
...
summary(mf_bl)
#View(na_count)
df_bl_mf <- scale(mf_bl[,c(2:33)]) # Standardize the data
#View(df_bl_mf)
...
final_data_vm$hclust <- groups_bl  #Adding hclusters to data.
#View(final_data_bl)
...
summary(pca_bl)
#DB scan
kNNdistplot(df_bl_mf, k =12) #k and minPts will be same, Considering PCA=11 which given 90% Cumulative Proportion
abline(h=7, col="red")
db_bl <- dbscan(df_bl_mf, eps=3.5, minPts=12)  #Need to perform pca to get the minpts, eps is from the above plot
db_bl
clusplot(df_bl_mf, db_bl$cluster, main='2D representation of the Cluster solution',
         color=TRUE, shade=TRUE,
         labels=2, lines=0)
hullplot(df_bl_mf, db_bl)
final_data_vm$dbscan<- db_bl$cluster
dbscan_results_vm<-aggregate(final_data_vm[, 2:33], list(final_data_vm$dbscan), mean)
#K means clustering
...
final_data_vm$kmeans <- k.means.fit_bl$cluster
...
write.table(final_data_vm, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/final_data_vm.csv", sep=",", row.names=F)
write.table(dbscan_results_vm, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/dbscan_results_vm.csv", sep=",", row.names=F)
