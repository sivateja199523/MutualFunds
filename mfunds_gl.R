...
rawData_gl <- fread("Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/mf_gl_mice.csv"
                    , sep=",", header=T, 
                    strip.white = T, na.strings = c("NA","NaN","","?"))
#View(rawData_bl)
mf_bl<-rawData_gl
final_data_gl <-rawData_gl
#View(mf_bl)
...
final_data_gl$hclust <- groups_bl  #Adding hclusters to data.
View(final_data_bl)
...
summary(pca_bl)
#DB scan
kNNdistplot(df_bl_mf, k =14) #k and minPts will be same, Considering PCA=13 which given 90% Cumulative Proportion
abline(h=7, col="red")
db_bl <- dbscan(df_bl_mf, eps=3, minPts=14)  #Need to perform pca to get the minpts, eps is from the above plot
db_bl
clusplot(df_bl_mf, db_bl$cluster, main='2D representation of the Cluster solution',
         color=TRUE, shade=TRUE,
         labels=2, lines=0)
hullplot(df_bl_mf, db_bl)
final_data_gl$dbscan<- db_bl$cluster
dbscan_results_gl<-aggregate(final_data_gl[, 2:37], list(final_data_gl$dbscan), mean)
#K means clustering
...
final_data_gl$kmeans <- k.means.fit_bl$cluster
...
write.table(final_data_gl, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/final_data_gl.csv", sep=",", row.names=F)
write.table(dbscan_results_gl, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/dbscan_results_gl.csv", sep=",", row.names=F)
library(fpc)
