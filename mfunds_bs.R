...
rawData_bs <- fread("Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/mf_bs_mice.csv"
                    , sep=",", header=T, 
                    strip.white = T, na.strings = c("NA","NaN","","?"))
rawData_bs<-select(rawData_bs,-price_earnings,-price_book,-price_sales,-price_cashflow)
View(rawData_bs)

mf_bl<-rawData_bs
final_data_bs <-rawData_bs
View(mf_bl)
...
final_data_bs$hclust <- groups_bl  #Adding hclusters to data.
#View(final_data_bl)
...
final_data_bs$dbscan<- db_bl$cluster
dbscan_results_bs<-aggregate(final_data_bs[, 2:33], list(final_data_bs$dbscan), mean)
#K means clustering
...
final_data_bs$kmeans <- k.means.fit_bl$cluster
...
write.table(final_data_bs, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/final_data_bs.csv", sep=",", row.names=F)
write.table(dbscan_results_bs, file="Desktop/Fall 2019/6211-Advanced BA/Group Project/mutual-funds-and-etfs/Mutualfunds/dbscan_results_bs.csv", sep=",", row.names=F)
library(fpc)
